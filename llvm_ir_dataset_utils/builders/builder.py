...
def get_corpus_size(corpus_dir):
    total_size = 0
    for bitcode_file in glob.glob(os.path.join(corpus_dir, "**/*.bc"), recursive=True):
...
def get_license_information(source_dir, corpus_dir):
    ...
    for license_description in license_files:
        ...
        with open(os.path.join(source_dir, license_file), "rb") as license_file_handle:
        ...
        with open(
            os.path.join(corpus_dir, new_license_path), "wb"
        ) as new_license_file_handle:
...
def parse_and_build_from_description(
    ...
    if corpus_description["build_system"] == "manual":
        ...
        build_dir = os.path.join(
            build_base_dir, corpus_description["folder_name"] + "-build"
        )
    ...
    if corpus_description["build_system"] == "cmake":
        ...
        build_log = cmake_builder.perform_build(
            configure_command_vector, build_command_vector, build_dir, corpus_dir
        )
        ...
        build_log = autoconf_builder.perform_build(
            configure_command_vector, build_command_vector, build_dir, corpus_dir
        )
        autoconf_builder.extract_ir(build_dir, corpus_dir, threads)
    elif corpus_description["build_system"] == "cargo":
        build_log = cargo_builder.build_all_targets(
            source_dir, build_dir, corpus_dir, threads, extra_env_variables, cleanup
        )
        if len(build_log["targets"]) == 0 and source_logs[-1]["type"] == "git":
            logging.warn(
                "Cargo builder detected no targets from git repository, "
        ...
        build_log = julia_builder.perform_build(
            corpus_description["package_name"], build_dir, corpus_dir, threads
        )
    ...
    with open(os.path.join(corpus_dir, "build_manifest.json"), "w") as build_manifest:
